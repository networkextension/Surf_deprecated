...
public class Tunnel: NSObject {
    ...
    func writeDataToTunnel(data: NSData, startingAtOffset: Int) -> Int {
        myLog("writeDataToTunnel called on abstract base class")
        return -1
    }
	...
	func serializeMessage(messageProperties: [String: AnyObject]) -> NSData? {
		...
		do {
			...
			myLog("Failed to create a data object from a message property list: \(messageProperties)")
		}
		return messageData
	}
	...
	func sendMessage(messageProperties: [String: AnyObject]) -> Bool {
        ...
        guard let messageData = serializeMessage(messageProperties) else {
            myLog("Failed to create message data")
            return false
        }
	...
	func sendData(data: NSData, forConnection connectionIdentifier: Int) {
		...
		if !sendMessage(properties) {
			myLog("Failed to send a data message for connection \(connectionIdentifier)")
		}
	}
	...
	func sendSuspendForConnection(connectionIdentifier: Int) {
		...
		if !sendMessage(properties) {
			myLog("Failed to send a suspend message for connection \(connectionIdentifier)")
		}
	}
	...
	func sendResumeForConnection(connectionIdentifier: Int) {
		...
		if !sendMessage(properties) {
			myLog("Failed to send a resume message for connection \(connectionIdentifier)")
		}
	}
	...
	public func sendCloseType(type: TunnelConnectionCloseDirection, forConnection connectionIdentifier: Int) {
		...
		if !sendMessage(properties) {
			myLog("Failed to send a close message for connection \(connectionIdentifier)")
		}
	}
	...
	func sendPackets(packets: [NSData], protocols: [NSNumber], forConnection connectionIdentifier: Int) {
		...
		if !sendMessage(properties) {
			myLog("Failed to send a packet message")
		}
	}
	...
	func handlePacket(packetData: NSData) -> Bool {
		...
		do {
			...
			myLog("Failed to create the message properties from the packet")
			return false
		}

		guard let command = properties[TunnelMessageKey.Command.rawValue] as? Int else {
			myLog("Message command type is missing")
			return false
		}
		guard let commandType = TunnelCommand(rawValue: command) else {
			myLog("Message command type \(command) is invalid")
			return false
		}
		...
		switch commandType {
			case .Data:
				...
				{
					myLog("Received data for connection \(connection?.identifier) from \(host):\(port)")
			...
			case .Close:
				...
				{
					myLog("\(connection?.identifier): closing \(closeDirection)")
					targetConnection.closeConnection(closeDirection)
				} else {
					myLog("\(connection?.identifier): closing reads and writes")
	...
	func handleMessage(command: TunnelCommand, properties: [String: AnyObject], connection: Connection?) -> Bool {
		myLog("handleMessage called on abstract base class")
		return false
	}
}
