...
int create_and_bind2(const char *port)
{
    int socket_handle;
    int error = 0;
    ...
    if(error == 0)
    {
        ...
        int opt = 1;
        setsockopt(socket_handle, SOL_SOCKET, SO_REUSEPORT, &opt, sizeof(opt));
        setsockopt(socket_handle, IPPROTO_TCP, TCP_NODELAY, &opt, sizeof(opt));
        setsockopt(socket_handle, SOL_SOCKET, SO_NOSIGPIPE, &opt, sizeof(opt));
...
int create_and_bind(const char *port) {
   ...
   //result = &hints;
    ...
    s = getaddrinfo(nil, port, &hints, &result);
#endif
...
int local_main ()
{
    ...
    int result = listen(listenfd, SOMAXCONN);
    if (result == -1) {
        NSLog(@"listen() error.");
        perror("listen");
        return 1;
